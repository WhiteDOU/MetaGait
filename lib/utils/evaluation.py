    ...
    if 'OUMVLP' not in dataset:
        ...
        for i in range(1):
            ...
            msg_mgr.log_info('NM: %.3f,\tBG: %.3f,\tCL: %.3f' % (
                ...
                de_diag(acc[1, :, :, i]),
                de_diag(acc[2, :, :, i])))
        result_dict["scalar/test_accuracy/NM"] = np.mean(de_diag(acc[0, :, :, 0], True))
        result_dict["scalar/test_accuracy/BG"] = np.mean(de_diag(acc[1, :, :, 0], True))
        result_dict["scalar/test_accuracy/CL"] = np.mean(de_diag(acc[2, :, :, 0], True))
        result_dict["scalar/test_accuracy/Mean"] = (result_dict["scalar/test_accuracy/NM"] + \
            result_dict["scalar/test_accuracy/BG"] + result_dict["scalar/test_accuracy/CL"])/3
        ...
        # result_dict["log/NM"] = de_diag(acc[0, :, :, 0], True)
        # result_dict["log/BG"] = de_diag(acc[1, :, :, 0], True)
        # result_dict["log/CL"] = de_diag(acc[2, :, :, 0], True)
        ...
        result_dict["scalar/test_accuracy/NM"] = np.mean(
            de_diag(acc[0, :, :, 0]))
        result_dict["log/NM"] = np.mean(de_diag(acc[0, :, :, 0]))
